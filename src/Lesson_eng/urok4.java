package Lesson_eng;

public class urok4 {
    /*
    They have a hospital in the centre jf thecity
    To unit -соединять
    United - соединительный
    my friend is from Beijing - мой друг из Пекина
    on holiday
    They are married - мы женаты
    To be married - быть женатым
    To marry - женится
    Oh no Look at the weather
    awtul - ужасная погода
    great - прекрасный
    really good - реально хорошо
    right -прав
    amazing - интересный
    To build - строить
    Building - здание
    Builder - Строитель
    favorite -фаворит
    Yes , you're right  Ты прав
    View - вид
    Look at the view  смотри на этот вид
    Beautiful -красивый
    match - определите / соедините
    a nurse -  медсестра
   an arfitect -архитектор
   job -работа
   What do you , do  - Че вы занимаетесь
   What are you ? - Кто вы (профЕСИЯ)
   00 - double zero
   Read -читать
   Complete - запомнить






     */
}
